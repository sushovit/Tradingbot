...
| 5 | **Regime block is per-strategy (boardroom 2026-07-28):** continuation setups (trend, momentum) are blocked when SPY is below its 20-EMA. `mean_reversion_reclaim` is **EXEMPT** — chop is its best cell (backtest 3y: **+0.38R** in chop vs +0.25R trending, PF 1.62). Exempt fires taken in chop are journaled with tag `chop_reclaim` for later analysis (`journal.chop_reclaim_report()`). | `streamlit_app.live_bot_worker`, config `spy_filter_exempt` | `test_rule5_exemption_is_configured`, `test_chop_reclaim_tag_is_an_acceptance_not_a_rejection` ✅ |
| 4 | **Targets are 3R (boardroom 2026-07-28):** `rr_ratio` 3.0 in both profiles; momentum and reclaim carry a 3R floor; reclaim keeps its structural prior-high target when that exceeds 3R. Backtest: 3R beat 2R on expectancy for every strategy, at the cost of win rate. | `bot_config.json`, strategy `MIN_TARGET_R` | `test_profiles_use_3r_targets_and_adx_30`, `test_reclaim_structural_target_wins_when_above_3r` ✅ |
